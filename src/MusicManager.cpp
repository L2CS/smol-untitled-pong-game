...
MusicManager::MusicManager()
    : audioFileSampleRate(44100), audioFileChannels(2), musicDuration(180.0f), visualizerTime(0.0f), songStartTime(0.0f), currentAudioPosition(0), bufferIndex(0), lastBeatIndex(0), currentBeatIntensity(0.0f), lastBeatTime(0.0f), soundsLoaded(false), musicPlaying(false), lastHitWasPaddle(false), lastPaddleHitTime(0.0f), fftw_input(nullptr), fftw_output(nullptr), fftw_plan_forward(nullptr)
    ...
    for (int i = 0; i < FFT_SIZE; i++) {
        ...
        if (sampleIndex < (int)audioFileData.size()) {
            if (audioFileChannels == 1) {
                ...
                float right = (sampleIndex + 1 < (int)audioFileData.size()) ? audioFileData[sampleIndex + 1] : 0.0f;
